...
const SelectInput = ({ options, state, setState }) => {
  ...
  return (
    <div className='relative w-fit rounded-md border border-gray-300 bg-gray-50 focus:border-gray-300'>
      ...
      <select className='block p-2.5 appearance-none relative pr-6 focus-visible:outline-none cursor-pointer sm:text-sm bg-transparent z-1 text-gray-500' value={state} onChange={handleChange}>
        {options.map((option) => (
